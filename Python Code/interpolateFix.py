...
def readFrames(file, data, header):
...
def scale_points(points):
    ...
    for point in points:
        ...
        scaled_x = int(((x + 32000) / 64000) * 4096)
        scaled_y = int(((y + 32000) / 64000) * 4096)
        scaled_points.append((scaled_x, scaled_y, laser))
    return scaled_points


def scale_data_points(data):
    for frame in data:
        # breakpoint()
        scaled_points = scale_points(frame["points"])
        frame["points"] = scaled_points


def generate_points_with_fixed_spacing(A, B, laser, spacing):
    x1, y1 = A
    x2, y2 = B
    dx = x2 - x1
    dy = y2 - y1
...
def interpolate_fixed_spacing(data, spacing=10):
    for frame in data:
        interpolated_points = []
        points = frame["points"]
        for i in range(len(points) - 1):
            ...
            interpolated_points.extend(
                generate_points_with_fixed_spacing(A, B, laser, spacing)
            )
...
def update(frame):
    ax.clear()
    breakpoint()
    segments = split_segments(frame)  # Adjust spacing
...
def readInfilePath():
    ...
    with open(file_path, "rb") as ILDA:
        ...
        while True:
            ...
            readFrames(ILDA, ILDAFileParsed, header)


readInfilePath()
scale_data_points(ILDAFileParsed)
interpolate_fixed_spacing(ILDAFileParsed)
# print(ILDAFileParsed[0])
...
ani = FuncAnimation(fig, update, frames=len(ILDAFileParsed), interval=5, repeat=True)
