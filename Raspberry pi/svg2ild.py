...
class SvgProcessor:
    ...
    def write_ild(self, params, rframe, center=True):
        ...
        for i, sample in enumerate(rframe):
            if sample.on:
                rframe = (
                    rframe[:i] + [sample] * params.extra_first_dwell + rframe[i + 1 :]
                )
                break
        ...
        for i, sample in enumerate(rframe):
            ...
            dout.append(
                (
                    (x + 2000),
                    4000 - (y + 2000),
                    random.choice([1, 2, 4, 5, 6, 7]) if sample.on else 0,
                )
            )
    ...
    def get_points(self):
        ...
        if verbose:
            print("Parse")
        frame = self.load_svg(self.filepath)
        if verbose:
            print("Done")
        ...
        return self.write_ild(params, rframe, center)
